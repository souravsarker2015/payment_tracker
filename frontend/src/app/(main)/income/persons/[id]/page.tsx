...
export default function PersonDetailPage() {
    ...
    useEffect(() => {
        fetchData();
    }, [personId, filterMode, customStartDate, customEndDate, selectedYear, selectedMonth]);
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const params: any = { person_id: personId };
            if (start_date) params.start_date = start_date;
            if (end_date) params.end_date = end_date;

            const [personRes, incomesRes, orgsRes] = await Promise.all([
    ...
    if (!person) {
        return <div className="text-center py-8">Person not found</div>;
    }
    ...
    const incomeByOrg = organizations.map(org => ({
        name: org.name,
        value: incomes.filter(inc => inc.organization_id === org.id).reduce((sum, inc) => sum + inc.amount, 0)
        ...
        <div className="space-y-6">
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <div className="flex items-center space-x-4 mt-1 text-sm text-gray-500">
                            {person.designation && <span>🏷️ {person.designation}</span>}
                            {person.phone && <span>📞 {person.phone}</span>}
                            <span className={`px-2 py-1 rounded-full text-xs font-semibold ${person.is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
            ...
            <div className="flex flex-wrap items-center gap-2 bg-white p-3 rounded-lg shadow-sm border border-gray-200">
                ...
                <button
                    ...
                    className="ml-auto inline-flex items-center px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-md hover:bg-green-700"
            ...
            <div className="bg-gradient-to-r from-green-500 to-green-600 rounded-xl shadow-lg p-6 text-white">
                <div className="flex items-center justify-between">
                    <div>
                        <p className="text-green-100 text-sm font-medium">Total Income (Selected Period)</p>
                        <p className="text-4xl font-bold mt-2">৳{totalIncome.toLocaleString()}</p>
                        <p className="text-green-100 text-sm mt-2">{incomes.length} transactions</p>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {incomeByOrg.length > 0 && (
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                        <h3 className="text-lg font-semibold text-gray-900 mb-4">Income by Organization</h3>
                        <div className="h-[300px]">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={incomeByOrg} layout="vertical">
                                    <CartesianGrid strokeDasharray="3 3" horizontal={false} />
                                    <XAxis type="number" />
                                    <YAxis dataKey="name" type="category" width={100} />
                                    <Tooltip formatter={(value) => `৳${Number(value).toLocaleString()}`} />
                                    <Bar dataKey="value" name="Income" fill="#10b981" radius={[0, 4, 4, 0]} />
                                </BarChart>
                            </ResponsiveContainer>
                        </div>
                    </div>
                                    ...
                                    <div className="flex items-center space-x-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-500">
                                                {org?.name || 'Unknown'} • {income.income_type}
                                            </p>
