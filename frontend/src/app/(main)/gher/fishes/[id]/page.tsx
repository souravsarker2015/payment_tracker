...
import { ArrowLeft, TrendingUp, DollarSign, Package, BarChart3, Calendar } from 'lucide-react';
...
export default function FishDetailPage() {
    ...
    const [stats, setStats] = useState<FishStats | null>(null);
    ...
    }, [fishId, filterMode, selectedYear, startYear, endYear, startDate, endDate]);
    ...
    const fetchStats = async () => {
        try {
            ...
            const res = await api.get(`/fishes/${fishId}/stats`, { params });
            setStats(res.data);

            // Fetch category if exists
            if (res.data.fish.category_id) {
                const categories = await api.get('/fish-categories');
                const cat = categories.data.find((c: any) => c.id === res.data.fish.category_id);
                setCategory(cat);
            }
            ...
            console.error('Failed to fetch fish stats', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    if (loading) {
        return <LoadingSpinner message="Loading fish details..." />;
    }

    if (!stats) {
        return (
            <div className="flex items-center justify-center min-h-screen">
                <div className="text-gray-500">Fish not found</div>
            </div>
        );
    }
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900">{stats.fish.name}</h1>
            ...
            {salesTrend.length === 0 && (
                <div className="bg-white p-12 rounded-xl border border-dashed border-gray-300 text-center">
                    ...
                    <p className="text-gray-500">No sales recorded for this fish yet.</p>
