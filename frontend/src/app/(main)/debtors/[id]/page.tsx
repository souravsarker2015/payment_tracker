...
const COLORS = ['#10b981', '#ef4444']; // Green for LEND, Red for RECEIVE
...
export default function DebtorDetailsPage() {
    ...
    const [type, setType] = useState<'LEND' | 'RECEIVE'>('LEND');
    ...
    const [modalType, setModalType] = useState<'LEND' | 'RECEIVE'>('LEND');
    ...
    const fetchData = async () => {
        try {
            const [debtorRes, transactionsRes] = await Promise.all([
                api.get(`/debtors/${id}`),
                api.get('/debtor-transactions')
            ]);
            setDebtor(debtorRes.data);
            const debtorTransactions = transactionsRes.data.filter((t: DebtorTransaction) => t.debtor_id === Number(id));
    ...
    const handleAddTransaction = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await api.post('/debtor-transactions', payload);
            setAmount('');
            setNote('');
            setTransactionDate('');
    ...
    const handleDeleteTransaction = async (transactionId: number) => {
        ...
        try {
            await api.delete(`/debtor-transactions/${transactionId}`);
            fetchData();
        } catch (error) {
    ...
    const openEditTransactionModal = (transaction: DebtorTransaction) => {
    ...
    const handleUpdateTransaction = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await api.put(`/debtor-transactions/${editingTransaction.id}`, payload);
    ...
    const getBalanceHistory = () => {
        ...
        return sortedTransactions.map(t => {
            if (t.type === 'LEND') balance += t.amount;
            if (t.type === 'RECEIVE') balance -= t.amount;
            return {
        ...
        <div>
            ...
            <div className="bg-white shadow rounded-lg p-6 mb-6">
                <div className="flex justify-between items-start">
                    <div>
                        <h1 className="text-2xl font-semibold text-gray-900">{debtor.name}</h1>
                        {debtor.phone && <p className="text-gray-500 mt-1">📞 {debtor.phone}</p>}
                        {debtor.debtor_type && <p className="text-gray-500 mt-1">🏷️ {debtor.debtor_type}</p>}
                        <div className="mt-4">
                ...
                <form onSubmit={handleAddTransaction} className="space-y-4 mt-4">
                    ...
                    <div>
                        ...
                        <select
                            id="type"
                            value={type}
                            onChange={(e) => setType(e.target.value as 'LEND' | 'RECEIVE')}
                            className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm text-black"
                        >
                            <option value="LEND">LEND (You give money)</option>
                            <option value="RECEIVE">RECEIVE (They pay you back)</option>
                        </select>
                    </div>
                    <div>
                ...
                <form onSubmit={handleUpdateTransaction} className="space-y-4 mt-4">
                    ...
                    <div>
                        ...
                        <select
                            id="modalType"
                            value={modalType}
                            onChange={(e) => setModalType(e.target.value as 'LEND' | 'RECEIVE')}
                            className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm text-black"
                        >
                            <option value="LEND">LEND (You give money)</option>
                            <option value="RECEIVE">RECEIVE (They pay you back)</option>
